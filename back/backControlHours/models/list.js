...
const deleteForId = (idUser,idList)=>{
    return new Promise((resolve, reject)=> {
        db.query("DELETE FROM lists where id = ? and fk_user = ?;", [idList, idUser],((error,result)=> {
            if (error) {
                reject(error);
            } else {
                resolve(result);
...
const create = ({title,descriptions, idUser})=>{
    return new Promise((resolve, reject)=> {
        db.query('INSERT INTO lists (title, descriptions, fk_user) values (?, ?, ?)', [title,descriptions,idUser],((error,result)=> {
            if (error) {
                reject(error);
            } else {
                resolve(result);
...
const updateById = ({title,descriptions,idUser,idList})=> {
    ...
    return new Promise((resolve, reject)=> {
        db.query('UPDATE lists SET title = ?, descriptions = ? where id =? and fk_user = ? ',[title,descriptions,idUser,idList],(error, result)=> {
            if (error) {
                reject(error);
            } else {
                resolve(result);
